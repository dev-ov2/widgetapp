...
func populate_list(user_widget_metadata: Array[WidgetMetadata], new_active_widget_metadata: Array[ActiveWidgetMetadata], on_enabled: Callable) -> void:
	...
	for metadata in user_widget_metadata:
		...
		widget_item_scene.set_data(metadata, active_widget_metadata, on_enabled)
